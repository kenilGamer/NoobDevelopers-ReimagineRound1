...
function Nav() {
  const [isMobile,
    setIsMobile] = useState(window.innerWidth <= 768);
  useEffect(() => {
    const handleResize = () => {
...
<div className="left_ w-full h-[15vw] flex  justify-between px-2">
      ...
      <img src="Rectangle 8.svg" className=" absolute -top-[1vw] w-full h-full object-fill" />
</div>
      <div className="h-full flex gap-4">
      <div className=" relative h-full  w-[10vw] ">
      <img src="/button/Group 23.svg" className="w-full h-full object-fill" />
      <img onClick={theme} src={dark_theme ? "light-mode-svgrepo-com.svg" : "moon.svg"} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full object-fill fill-white scale-50" />
    </div>
       <div className=" relative h-full  w-[10vw]">
      <img src="/button/Group 23.svg" className="w-full h-full object-fill" />
      <img onClick={() => setSd(true)} src="three_line.svg" className=" absolute top-1/2 left-1/2 -translate-x-1/2  -translate-y-1/2 w-full h-full object-fill fill-white scale-50" />
</div>
</div>
</div> < />
);
